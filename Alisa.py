...
def clean_nickname(nick):
    """Очищает никнейм от нежелательных символов"""
...
async def show_help(ctx):
    ...
    embed.add_field(
        name="ya!say [текст]",
        value="Озвучить произвольный текст (только для пользователей с разрешением)",
        inline=False
    )
    ...
    embed.set_footer(text="Алиса | Голосовой помощник", icon_url=bot.user.avatar.url if bot.user.avatar else None)
...
async def on_voice_state_update(member, before, after):
    ...
    if after.channel and after.channel.id == VOICE_CHANNEL_ID and (
            not before.channel or before.channel.id != VOICE_CHANNEL_ID):
